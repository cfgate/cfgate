...
func TestBuildRulesRejectsBackendTLSOriginProtocolHTTPAnnotation(t *testing.T) {
	ctx := context.Background()
	scheme := controllerTestScheme(t)
	route := routeForBackendTest("route", "apps", "app")
	route.Annotations = map[string]string{
	...
	if err == nil {
		t.Fatal("buildRulesFromHTTPRouteForHostnamesWithRuntime() error = nil, want BackendTLSPolicy invariant error")
	}
	for _, want := range []string{"apps/route", annotations.AnnotationOriginProtocol + "=http", "apps/tls-app", "HTTPS"} {
...
func TestBuildRulesAllowsBackendTLSOriginProtocolHTTPSAnnotation(t *testing.T) {
	ctx := context.Background()
	scheme := controllerTestScheme(t)
	route := routeForBackendTest("route", "apps", "app")
	route.Annotations = map[string]string{
	...
	if len(rules) != 1 || rules[0].Service != "https://app.apps.svc.cluster.local:8443" {
		t.Fatalf("rules = %#v, want one HTTPS BackendTLSPolicy rule", rules)
	}
}

func TestBuildRulesAllowsOriginProtocolHTTPAnnotationWithoutBackendTLS(t *testing.T) {
	ctx := context.Background()
	route := routeForBackendTest("route", "apps", "app")
	route.Annotations = map[string]string{
		annotations.AnnotationOriginProtocol: "http",
	...
	if len(rules) != 1 || rules[0].Service != "http://app.apps.svc.cluster.local:8443" {
		t.Fatalf("rules = %#v, want one HTTP rule", rules)
	}
}

func TestRouteH2cValidationRejectsServiceScheme(t *testing.T) {
	route := routeForBackendTest("route", "apps", "app")
	config := &cloudflare.OriginRequestConfig{H2cOrigin: true}
	for _, service := range []string{
		"https://app.apps.svc.cluster.local:8443",
		"wss://app.apps.svc.cluster.local:8443",
	} {
		if err := validateRouteOriginRequest(route, config, service, false); err == nil || !strings.Contains(err.Error(), "h2cOrigin requires cleartext HTTP origin service") {
			t.Fatalf("validateRouteOriginRequest(%q) error = %v, want service scheme rejection", service, err)
		}
	}
}

func TestApplyOriginPolicyRequiresConfigWhenPolicyExists(t *testing.T) {
	policy := originPolicyForTest("origin", time.Now())
	_, err := applyOriginPolicy(nil, policy, nil)
	if err == nil || !strings.Contains(err.Error(), "requires non-nil origin request config") {
		t.Fatalf("applyOriginPolicy() error = %v, want nil config contract error", err)
	}
}

func TestApplyBackendTLSPolicyRequiresConfigWhenPolicyExists(t *testing.T) {
	policy := backendTLSPolicyForTest("tls-app", "apps", "app", "app.example.com", "ca-app")
	_, err := applyBackendTLSPolicy(nil, policy, nil)
	if err == nil || !strings.Contains(err.Error(), "requires non-nil origin request config") {
		t.Fatalf("applyBackendTLSPolicy() error = %v, want nil config contract error", err)
	}
}
